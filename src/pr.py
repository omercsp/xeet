...
def create_print_func(dflt_color: str, level: int, dflt_console: Console = _stdout) -> Callable:
    def print_func(*args, **kwargs) -> None:
        ...
        if not kwargs.pop("pr_cond", True):
            return
        #  Allow disabling of colors (markup) in the print function. Useful when printing
        #  a string that looks like a marup but should not be interpreted as such (e.g. [my brackets
        #  string])
        markup = kwargs.pop("pr_markup", None)
        ...
        console.print(*args, **kwargs, style=color, markup=markup)
    return print_func
...
def pr_generic(*args, **kwargs) -> None:
    severity = kwargs.pop("severity", LogLevel.INFO)
    if severity == LogLevel.INFO:
        pr_info(*args, **kwargs)
    elif severity == LogLevel.WARN:
        pr_warn(*args, **kwargs)
    elif severity == LogLevel.ERROR:
        pr_error(*args, **kwargs)
...
def pr_obj(obj: Any, **kwargs) -> None:
    ...
    if isinstance(obj, (list, dict)):
        ...
        elif print_type == DictPrintType.JSON:
...
__all__ = ["pr_info", "pr_warn", "pr_error", "pr_ok", "pr_header", "pr_obj", "clear_printed_line",
           "pr_generic", "set_print_level", "mute_prints", "disable_colors", "colors_enabled",
