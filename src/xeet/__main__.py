...
from xeet.cli_printer import CliPrinterTestTimingOpts, CliPrinterShowOpts
...
_DISPLAY_COMPONENTS = CliPrinterShowOpts.components()
...
def _validated_display_component_list(s: str) -> list[str]:
    ...
    if illegal_tokens:
        raise argparse.ArgumentTypeError(
            ...
            f"Allowed tokens are: {', '.join(CliPrinterShowOpts.components())}"
        )
    return tokens
...
def parse_arguments() -> argparse.Namespace:
    ...
    run_parser.add_argument('--test-timing-type',
                            choices=[s.value for s in CliPrinterTestTimingOpts],
...
def _display_settings(args: argparse.Namespace) -> CliPrinterShowOpts:
    ...
    if args.test_timing_type is not None:
        opts.test_timing_type = CliPrinterTestTimingOpts(args.test_timing_type)
    return opts
