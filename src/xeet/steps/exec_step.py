...
from xeet.settings import setting_value
...
class _ExecStepSettingsModel(BaseModel):
    ...
    default_shell_path: str = Field("", alias="default_shell_path", min_length=1)
...
def _settings() -> _ExecStepSettingsModel:
    ...
    if _settings_desc is not None:
        _settings_desc = {}
    try:
        return _ExecStepSettingsModel(**_settings_desc)
    except ValidationError as e:
        raise XeetException(f"Error parsing setting 'exec_step': {pydantic_errmsg(e)}")
