...
class ExecStep(Step):
    ...
    def setup(self, **kwargs) -> None:  # type: ignore
        ...
        if self.env_file is not None:
            ...
            self.log_info(f"Environment file will be read from '{self.env_file}'")
        ...
        if self.output_behavior == _OutputBehavior.Unify:
            self.log_info(f"Unified output will be written to '{self.stdout_file}'", dbg_pr=False)
        else:
            ...
            self.log_info(f"Stdout will be written to '{self.stdout_file}'", dbg_pr=False)
            self.log_info(f"Stderr will be written to '{self.stderr_file}'", dbg_pr=False)
    ...
    def _run(self, res: ExecStepResult) -> bool:  # type: ignore
        ...
        if self.debug_mode:
            tails.append(FileTailer(self.stdout_file, pr_func=self.log_info))
            if self.output_behavior == _OutputBehavior.Split:
                tails.append(FileTailer(self.stderr_file, pr_func=self.log_info))
        try:
