...
def _read_config_model(file_path: str,
                       xvars: XeetVars,
                       root_dir: str = "",
    ...
    if model.includes:
        ...
        for i in includes[::-1]:
            inc_model = _read_config_model(i, xvars, root_dir, included)
...
def read_config_file(file_path: str) -> Xeet:
    xvars = XeetVars()
    model = _read_config_model(file_path, xvars)
    return Xeet(model, xvars)
