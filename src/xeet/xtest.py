...
class XtestModel(BaseModel):
    ...
    def inherit(self, other: "XtestModel") -> None:
        ...
        self.pre_run = _inherit_steps(self.pre_run, other.pre_run, self.pre_run_inheritance)
        self.run = _inherit_steps(self.run, other.run, self.run_inheritance)
    ...
    def _init_step_list(self, step_list: list[dict] | None, prefix: str) -> list[XStep] | None:
        ...
        for index, step_desc in enumerate(step_list):
            try:
                ...
                step = step_class(step_model, self.xdefs, f"{id_base}/{index}")
                ret.append(step)
