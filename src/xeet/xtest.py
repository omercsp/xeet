...
class TestStatus(Enum):
    ...
    Init_error = auto()
    Pre_run_error = auto()
    Run_error = auto()  # This isn't a test failure per se, but a failure to run the test
    Verfication_run_err = auto()
    Failed = auto()
...
class Xtest:
    ...
    def run(self) -> TestResult:
        res = TestResult()
        if self.init_err:
            res.status = TestStatus.Init_error
            return res
        if self.skip:
        ...
        if not self.cmd:
            ...
            res.status = TestStatus.Run_error
    ...
    def _pre_test(self, res: TestResult, env: dict) -> None:
        ...
        try:
            ...
            res.status = TestStatus.Pre_run_error
            res.status_reason = f"Pre-test run failure: {e}"
            res.pre_cmd_rc = -1
            return
    ...
    def _run(self, res: TestResult, env: dict) -> None:
        ...
        try:
            ...
            res.status = TestStatus.Run_error
            res.status_reason = f"Test command run error - {e}"
    ...
    def _run_verify_cmd(self, res: TestResult, env: dict) -> None:
        ...
        try:
            ...
            res.status = TestStatus.Verfication_run_err
    ...
    def _verify(self, res: TestResult, env: dict) -> None:
        ...
        if res.status == TestStatus.Failed and self.expected_failure:
            res.status = TestStatus.Expected_failure
        elif res.status == TestStatus.Undefined and self.expected_failure:
