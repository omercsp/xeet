...
class CliPrinter(LockableEventReporter):
    ...
    def on_test_end(self, test_res: TestResult) -> None:
        ...
        msg += f"[{stts_str}]"
        if self.test_timing_opt != CliPrinterTestTimingOpts.NoTime:
    ...
    def on_matrix_start(self) -> None:
        ...
        if self.mtrx_count == 1:
            pr_info(self._iter_header(self.iteration_index, -1, False))
        else:
            pr_info(self._iter_header(self.iteration_index, self.mtrx_prmttn_index, False))
    ...
    def _iter_header(self, iter_i: int, mtrx_i: int, show_time: bool) -> str:
        ret = ""
        ...
        #  if show_time and self.summary_timings:
        #      ret += f" ({self.run_res.iter_results[iter_i].duration_str})"
        return ret
    ...
    def on_run_end(self) -> None:
        ...
        if self.summary_opt != CliPrinterSummaryOpts.SummaryOnly:
            msg = "Summary"
            if self.summary_timings:
                msg += f" ({self.run_res.duration_str})"
            msg += ":"
            pr_info(title(msg, color=XColors.Bold))
        ...
        for iter_i, iter_res in enumerate(self.run_res.iter_results):
            ...
            for mtrx_i, mtrx_res in enumerate(iter_res.mtrx_results):
                ...
                if self.concise:
                    continue
                header = self._iter_header(iter_i, mtrx_i, True)
                header = title(header, '-', newline_prefix=False, color=_ITERATION_COLOR)
                pr_info(header)
