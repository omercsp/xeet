...
class Test:
    def __init__(self, model: TestModel, rti: RuntimeInfo) -> None:
        ...
        self._pre_steps = self._phase_wrapper(self.pre_run_steps, self._pre_test)
        self._main_run = self._phase_wrapper(self.run_steps, self._main_run)
        self._post_test = self._phase_wrapper(self.post_run_steps, self._post_test)
    ...
    def _phase_wrapper(self, step_list: _StepList, func: Callable) -> Callable:
        @wraps(func)
        def wrapper(res: TestResult) -> None:
            self.rti.notifier.on_phase_start(self, step_list.name, len(step_list.steps))
            func(res)
            self.rti.notifier.on_phase_end(self, step_list.name, len(step_list.steps))
