...
class Test:
    ...
    def _init_phase(self, steps: list[dict], name: str, short_name: str, stop_on_err: bool
        ...
        for index, step_desc in enumerate(steps):
            try:
                self.notify(f"initializing {name} step {index}")
                ...
                self.warn(f"error initializing {name}step {index}: {e}")
    ...
    def run(self) -> TestResult:
        ...
        if not self.main_phase.steps:
            self.notify("no commands for test, will not run", dbg_pr=True)
            return TestResult(TestStatus(TestPrimaryStatus.NotRun), status_reason="No commands")
    ...
    def notify(self, msg, *args, **kwargs) -> None:
