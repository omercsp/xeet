...
_SUB_STATUS_TEXT = {
    ...
    TestSubStatus.PreTestErr: "Pre-test error",
    TestSubStatus.TestErr: "Test error",
...
class TestResult:
    ...
    def error_summary(self) -> str:
        ret = ""
        if self.sub_status == TestSubStatus.PreTestErr:
        ...
        elif self.status == TestStatus.Failed or self.status == TestStatus.NotRun:
...
TestFullStatus = tuple[TestStatus, TestSubStatus]
...
class MtrxResult:
    def __init__(self, mp: MatrixPermutation, mpi: int) -> None:
        ...
        self.status_results_summary: dict[TestFullStatus, list[str]] = {}
        self.results = {}
