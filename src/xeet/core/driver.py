...
def xeet_init(file_path: str, debug_mode=False, reporters: list[RunReporter] = list()) -> _Driver:
    if not file_path:
    ...
    rti = RuntimeInfo(file_path, debug_mode, notifier)
    log_info(f"Main Xeet configuration file: {rti.xeet_file_path}")
    log_info(f"Root directory: {rti.root_dir}")
    log_info(f"Current working directory: {rti.cwd}")
    log_info(f"Expected output directory: {rti.expected_output_dir}")
    model = _read_conf_file(rti.xeet_file_path, rti.xvars, rti.root_dir)
    rti.set_defs(model.model_dump(by_alias=True))
    return _Driver(model, rti)
