...
from .xres import (TestResult, TestStatus, TestSubStatus, RunResult, IterationResult,
                   MtrxResult, status_as_str)
...
def run_tests(conf: str,
    ...
    for iter_n in range(iterations):
        ...
        for mtrx_i, mtrx in enumerate(matrix.permutations()):
            ...
            for status, sub_status in mtrx_res.status_results_summary:
                test_names = mtrx_res.status_results_summary[(status, sub_status)]
                if not test_names:
                    continue
