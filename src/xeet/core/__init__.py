...
class TestsCriteria:
    ...
    hidden_tests: bool = False  # If True, include abstract tests (unrunabble)
    ...
    def __str__(self) -> str:
        ...
        filter_str = ", ".join(map(str, sorted(self.prmttn_idxs_inc))) \
            if self.prmttn_idxs_inc else "<none>"
        lines.append(f"Permutations indexes included: {filter_str}")

        filter_str = ", ".join(map(str, sorted(self.prmttn_idxs_exc))) \
            if self.prmttn_idxs_exc else "<none>"
        lines.append(f"Permutations indexes excluded: {filter_str}")

        lines.append("Abstract tests are {}included".format("" if self.hidden_tests else "not "))
