...
from .console_printer import ConsolePrinter, ConsoleDisplayOpts, ConsolePrinterVerbosity
...
def run_tests(conf_path: str, run_settings: XeetRunSettings, display_opts: ConsoleDisplayOpts
              ) -> int:
    ...
    with Live(console=stdout(), refresh_per_second=4, transient=False) as live:
        if run_settings.debug:
            settings.reporters.append(DebugPrinter())
        else:
