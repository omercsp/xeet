from xeet.core.xtest import Xtest, TestPrimaryStatus
from xeet.core.xres import (TestResult, XStepResult, IterationResult, RunResult, TestPrimaryStatus,
                            TestSecondaryStatus, TestStatus)
...
class CliPrinter(RunReporter):
    ...
    @staticmethod
    def status_colors(status: TestPrimaryStatus) -> tuple[str, str]:
    ...
    def on_test_end(self, test: Xtest | None = None, test_res: TestResult | None = None, **_
        ...
        stts_str = _colorize_str(status_text, self.status_colors(test_res.status.primary))
        msg += f"[{stts_str}]"
        if status_suffix:
    ...
    def on_iteration_end(self) -> None:
        ...
        def summarize_test_list(status: TestStatus, names: list[str]) -> None:
            if not names:
                return
            colors = self.status_colors(status.primary)
            test_list_str = ", ".join(names)
            self.console.print(f"{colors[0]}{status}{colors[1]}: {test_list_str}")
        ...
        assert isinstance(self.iter_res, IterationResult)
        sttss = sorted(self.iter_res.status_results_summary.keys(), key=lambda x: x.primary.value)
        for s in sttss:
            test_names = self.iter_res.status_results_summary[s]
            summarize_test_list(s, test_names)
        self.live.update("")
        self.console.print()
