...
from xeet import XeetDefs, glbl_var_name
...
class Driver:
    def __init__(self, model: XeetModel, xvars: XeetVars) -> None:
        self.model = model
        self.xvars = xvars
        self.defs = XeetDefs(model.model_dump(by_alias=True), model.root_dir, xvars)
...
def xeet_init(file_path: str) -> Driver:
    ...
    xvars = XeetVars(start_vars={
        glbl_var_name("CWD"): cwd,
        glbl_var_name("ROOT"): root_dir
    })
    model = _read_conf_file(file_path, xvars, root_dir)
