...
from xeet import RunReporter, XeetDefs, TestCriteria
...
def xeet_init(file_path: str, debug_mode=False, reporter: RunReporter | None = None) -> Driver:
    if not file_path:
    ...
    if reporter is None:
        reporter = RunReporter(iterations=1)  # defult to null reporter
    xdefs = XeetDefs(file_path, debug_mode, reporter)
