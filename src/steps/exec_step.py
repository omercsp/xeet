from xeet.common import (text_file_tail, in_windows, FileTailer, validate_types, StrFilterData,
                         filter_str, validate_str)
from xeet.pr import pr_info
from xeet.core.step import Step, StepModel, StepResult
from xeet import XeetException
from pydantic import field_validator, ValidationInfo, model_validator, Field, AliasChoices
...
class ExecStepModel(StepModel):
    ...
    cmd: str | None = None
    use_shell: bool = Field(False, validation_alias=AliasChoices("use_shell", "shell"))
...
class ExecStep(Step):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.timeout = self.exec_model.timeout
    ...
    def setup(self, **kwargs) -> None:  # type: ignore
        ...
        self.timeout = self.xvars.expand(self.exec_model.timeout)
        if self.timeout is not None and not validate_types(self.timeout, [int, float]):
            raise XeetException(f"Invalid timeout value '{self.timeout}'")
    ...
    def _read_env_vars(self) -> dict:
        ret = {}
        if "use_os_env" in self.exec_model.user_keys() and self.exec_model.use_os_env:
    ...
    def _run(self, res: ExecStepResult) -> bool:  # type: ignore
        ...
        if self.debug_mode:
            tails.append(FileTailer(self.stdout_file, pr_func=self.output))
            if self.output_behavior == _OutputBehavior.Split:
                tails.append(FileTailer(self.stderr_file, pr_func=self.output))
        try:
    ...
    def _verify_output(self, res: ExecStepResult) -> None:
        ...
        def _compare_std_file(name, actual_output_file, expected_content):
            with open(actual_output_file, "r") as f:
                actual_content = f.read()
                if self.output_filters:
                    self.notify(f"applying filters to {name} - {self.output_filters}")
                    actual_content = filter_str(actual_content, self.output_filters)
                    with open(f"{actual_output_file}.filtered", "w") as filtered_f:
                        filtered_f.write(actual_content)
                actual_content = actual_content.split("\n")
            diff = difflib.unified_diff(
                expected_content,  # List of lines from file1
                actual_content,  # List of lines from file2
        ...
        self.notify(f"expected stdout file: {self.expected_stdout_file}", dbg_pr=False)
    ...
    def _stop(self) -> None:
        if self.p and self.p.poll() is None:
            self.notify(f"Stopping process... {self.p.pid}")
            self.p.terminate()
    ...
    #  Internal knob controlling debug output spacing, not a test setting
    _HIDDEN_KEYS = {"debug_new_line"}

    def _details_keys(self, full: bool) -> set[str]:
        ret = super()._details_keys(full)
        if full:
            ret |= set(ExecStepModel.model_fields.keys())
        else:
            ret |= self.exec_model.model_fields_set
        return ret - self._HIDDEN_KEYS

    def _field_details_order(self) -> list[str]:
        return ["cmd", "cwd", "use_shell", "shell_path", "env_file",
                "env", "timeout", "allowed_rc", "output_behavior", "stdout_file",
                "stderr_file", "expected_stdout", "expected_stdout_file",
                "expected_stderr", "expected_stderr_file", "output_filters",
                "stop_process_wait"]

    def _detail_value(self, key: str, printable: bool, setup: bool = False, **_) -> Any:
        if key == "env":
            env = self.env if setup else self.exec_model.env
            if not env or not printable or isinstance(env, str):
                return self._printable_value(env) if printable else env
            return "\n".join([f"{k}='{v}'" for k, v in env.items()])

        if key == "output_filters":
            filters = self.output_filters if setup else self.exec_model.output_filters
            if not filters or not printable or isinstance(filters, str):
                return super()._detail_value(key, printable)
            ret = ""
            for i, f in enumerate(filters):
                ret += (f"[{i}] From: '{f.from_str}'\n"
                        f"    To: '{f.to_str}'\n"
                        f"    Regex: {f.regex}\n")
            return ret

        if not setup or key in self._DFLT_KEYS:
            return super()._detail_value(key, printable)
        try:
            value = getattr(self, key)
        except AttributeError:
            #  Not resolved by setup(); report the raw model value instead
            return super()._detail_value(key, printable)
