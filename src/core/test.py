...
from xeet.common import (XeetException, XeetVars, pydantic_errmsg, XeetToken, XeetVarsModel)
...
_EMPTY_STR = ""
_TEST_NANE_PATTERN = r"^[a-zA-Z0-9_-]*$"
...
class TestModel(BaseModel):
    ...
    base: str = Field("", pattern=_TEST_NANE_PATTERN, min_length=1)
    abstract: bool = False
    short_desc: str = Field(_EMPTY_STR, max_length=75)
    long_desc: str = _EMPTY_STR
    groups: list[XeetToken] = Field(default_factory=list)
    ...
    def set_parent(self, parent: "TestModel") -> None:
        self.parent = parent
        if parent.error:
            self.error = f"Error in parent '{parent.name}': {parent.error}"
            return
        if self.inherit_matrix and parent.matrix:
            self.matrix = {**parent.matrix, **self.matrix}
        if self.inherit_matrix or self.inherit_variables:
            intersection = set(self.variables().keys()).intersection(set(self.matrix.keys()))
            if intersection:
                self.error = (
                    f"After inheriting '{parent.name}', the following tokens are used for "
                    f"both variables and matrix: {intersection}. This is not allowed, as "
                    "it may lead to unexpected results."
                )

    def variables(self) -> XeetVarsModel:
        if self.inherit_variables and self.parent:
            parent_vars = self.parent.variables()
            return XeetVarsModel(**{**parent_vars, **self.var_map})
        return self.var_map

    def _step_list(self, steps_key: str) -> list:
        self_steps = getattr(self, steps_key)
        inherit_method = getattr(self, f"{steps_key}_inheritance")
        if not self.parent or (inherit_method == StepsInheritType.Replace and steps_key in
                               self.model_fields_set):
            return self_steps

        if inherit_method == StepsInheritType.Append:
            ret = self.parent._step_list(steps_key) + self_steps
        else:
            ret = self_steps + self.parent._step_list(steps_key)
        return ret

    def pre_run_steps(self) -> list[Any]:
        return self._step_list("pre_run")

    def run_steps(self) -> list[Any]:
        return self._step_list("run")
...
class Test:
    ...
    def __init__(self, model: TestModel, rti: RuntimeInfo, index: int = -1) -> None:
        ...
        self.obtained_vars: list[str] = []
        self.prmmtn_tests: list[Test] = list()

        self.base = self.model.base
        self.error = model.error
        self.steps_built = False
        self.output_dir = _EMPTY_STR
        self.stop_requested = False
        self.prmttn_values: dict[str, Any] = dict()
        self.mtrx_base_name = self.name
        self.permutation_index = -1

        parts = self.name.split(":")
        if len(parts) == 2:
            try:
                self.mtrx_base_name = parts[0]
                self.permutation_index = int(parts[1])
                if self.permutation_index < 0:
                    raise ValueError()
            except ValueError:
                self.error = f"Invalid permutation name '{self.name}'"
                return

        try:
            variables = {**model.variables(), **model.prmttn}
        ...
        self.xvars.set_vars({
            ...
            system_var_name("TEST_BASE_NAME"): self.mtrx_base_name,
        })

    @cached_property
    def is_permutation(self) -> bool:
        return self.permutation_index >= 0

    #  Steps are built on demand, not on test creation. Building a step resolves its
    #  'base' reference and validates its model, which is wasted work for tests that
    #  won't run. Callers that need the steps, or need step initialization errors to
    #  show up in self.error, must call this first. It's idempotent.
    def build_steps(self) -> None:
        if self.steps_built:
            return
        self.steps_built = True
        if self.error:
            self.notify(f"skipping phase initialization due to error - {self.error}", dbg_pr=True)
            return

        phase_steps = ((self.pre_phase, self.model.pre_run_steps()),
                       (self.main_phase, self.model.run_steps()),
                       (self.post_phase, self.model.post_run_steps()))
        for phase, steps in phase_steps:
            self._init_phase_steps(phase, steps)
            if self.error:
                return
    ...
    def setup(self, setup_steps: bool) -> None:
        if self.error:
            self.notify(f"skipping setup due to error - {self.error}", dbg_pr=True)
            return
        self.notify("setting up", dbg_pr=False)
    ...
    #  Resolves each step's variables and output paths. Unlike build_steps() this runs
    #  once per iteration, since the output directory changes between iterations.
    def setup_steps(self) -> None:
        if self.error:
            self.notify(f"skipping step initialization due to error - {self.error}", dbg_pr=True)
            return

        step_xvars = XeetVars(parent=self.xvars)
        try:
            for phase in (self.pre_phase, self.main_phase, self.post_phase):
    ...
    def release_resources(self) -> None:
        ...
        if self.obtained_vars:
            self.xvars.pop_vars(self.obtained_vars)
            self.obtained_vars.clear()

    def obtain_resources(self) -> bool:
        try:
            for req in self.model.resource_list():
                self.notify(f"obtaining resource '{req.pool}'")
                if req.names:
                    names = [n for n in req.names]
                    obtained = self.rti.obtain_resource_list(req.pool, names)
                else:
                    obtained = self.rti.obtain_resource_list(req.pool, req.count)

                if not obtained:
                    self.notify(f"resource '{req.pool}' not available")
                ...
                if req.as_var:
                    ...
                    self.obtained_vars.append(req.as_var)
        except XeetException as e:
    ...
    def run(self) -> TestResult:
        ...
        if not self.main_phase.steps:
            self.notify("no command for test, will not run", dbg_pr=True)
            res.status = TestStatus(TestPrimaryStatus.NotRun)
    ...
    def _gen_step_model(self, desc: dict, included: set[str] | None = None) -> StepModel:
        ...
        try:
            ...
            if base_step_model:
                step_model.set_parent(base_step_model)
