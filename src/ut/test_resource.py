...
def test_get_resource_by_name(xut: XeetUnittest):
    xut.add_resource("res1", "r0", "simple")
    ...
    results = xut.run_tests_list([TEST3, TEST4, TEST5], jobs=3)
    for res in results:
        xut.update_test_res_test(expected, res.test.name)
        assert_test_results_equal(res, expected)
...
def test_get_resource_by_count(xut: XeetUnittest):
    ...
    expected = gen_test_result(status=PASSED_TEST_STTS, main_results=[DUMMY_OK_STEP_RES])
    results = xut.run_tests_list([TEST0, TEST1], jobs=3)
    for res in results:
        xut.update_test_res_test(expected, res.test.name)
        assert_test_results_equal(res, expected)
    ...
    expected = gen_test_result(
        status=TestStatus(TestPrimaryStatus.NotRun, TestSecondaryStatus.InitErr))
    results = xut.run_tests_list([TEST2], jobs=3)
    for res in results:
        xut.update_test_res_test(expected, res.test.name)
        assert_test_results_equal(res, expected)
...
def test_resource_as_var_multi_iteration(xut: XeetUnittest):
    xut.add_resource("res1", "r0", "hello_res")
    step = gen_dummy_step_desc(dummy_val0="{my_res.r0}")
    xut.add_test(TEST0, run=[step], resources=[gen_resouce_req("res1", names=["r0"],
                 as_var="my_res")], save=True)
...
def test_resource_inheritance(xut: XeetUnittest):
    xut.add_resource("res1", "", "val1", reset=True)
    xut.add_resource("res2", "", "val2")

    # Base test requires res1
    xut.add_test(TEST0, abstract=True, resources=[gen_resouce_req("res1", count=1)],
                 run=[DUMMY_OK_STEP_DESC])
    # Child test inherits res1 requirement
    xut.add_test(TEST1, base=TEST0)
    # Child test overrides with res2 requirement
    xut.add_test(TEST2, base=TEST0, resources=[gen_resouce_req("res2", count=1)])
    # Child test clears resource requirements
    xut.add_test(TEST3, base=TEST0, resources=[], save=True)

    expected = gen_test_result(status=PASSED_TEST_STTS, main_results=[DUMMY_OK_STEP_RES])
    results = xut.run_tests_list([TEST1, TEST2, TEST3], jobs=2)
    for res in results:
        xut.update_test_res_test(expected, res.test.name)
        assert_test_results_equal(res, expected)
