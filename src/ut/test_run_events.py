...
class _Reporter(RunReporter):
    ...
    # Test events
    def _std_test_check(self, test: Xtest | None = None, fname: str = "") -> bool:
        if not fname:
    ...
    def on_test_start(self, test: Xtest | None = None, **_) -> None:
        ...
        self.tests_acc[test.name].start_count += 1

    def on_test_end(self, test: Xtest | None = None, **_) -> None:
        if not self._std_test_check(test):
            return
        assert test is not None
        self.tests_acc[test.name].end_count += 1

    def _std_phase_check(self, test: Xtest | None = None, phase_name: str = "", fname: str = ""
                         ) -> bool:
        if not fname:
            fname = inspect.currentframe().f_back.f_code.co_name  # type: ignore
    ...
    def on_phase_start(self, test: Xtest | None = None, phase_name: str = "", **_) -> None:
        if not self._std_phase_check(test, phase_name):
            return
        assert test is not None
        self.tests_acc[test.name].phases_started.append(phase_name)

    def on_phase_end(self, test: Xtest | None = None, phase_name: str = "", **_) -> None:
        if not self._std_phase_check(test, phase_name):
            return
        assert test is not None
    ...
    # Step events
    def _std_step_check(self, test: Xtest | None = None, step: XStep | None = None,
                        phase_name: str = "") -> bool:
        fname = inspect.currentframe().f_back.f_code.co_name  # type: ignore
    ...
    def on_step_start(self, test: Xtest | None = None, step: XStep | None = None,
    ...
    def on_step_end(self, test: Xtest | None = None, phase_name: str = "",
                    step: XStep | None = None,  **_) -> None:
        if not self._std_step_check(test, step, phase_name):
            return
        assert test is not None
    ...
    def _test_run_events(self, threads: int):
        ...
        for i in [1, 2, 3]:
            ...
            for acct in acc_test.values():
                self.assertEqual(acct.start_count, i)
                self.assertEqual(acct.end_count, i)
