...
from xeet.config import (ConfigModel, TestCriteria, XeetIncludeLoopException, Config,
                         read_config_file, _read_config_model)
...
class TestConfig(DummyTestConfig):
    def test_config_model_inclusion(self):
        ...
        model = _read_config_model(conf1.file_path)
        ...
        conf2.save()
        model = _read_config_model(conf2.file_path)
        self.assertIsInstance(model, ConfigModel)
