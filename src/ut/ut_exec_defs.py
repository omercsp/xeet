...
from xeet.core.xres import TestResult, TestStatus, TestSubStatus, XStepListResult
import os
...
def gen_exec_result(status: TestStatus, sub_status: TestSubStatus = TestSubStatus.Undefined,
                    **kwargs) -> TestResult:
    step_res_desc = ExecStepResult(**kwargs)
    return TestResult(status=status, sub_status=sub_status,
                      run_res=XStepListResult(results=[step_res_desc]))


GOOD_EXEC_RES = gen_exec_result(status=TestStatus.Passed, rc=0, rc_ok=True, completed=True)
...
__all__ = ["SHOWENV_CMD", "ECHOCMD", "PWD_CMD", "BAD_CMD", "OUTPUT_CMD", "RAND_RC_CMD",
           "GOOD_EXEC_RES", "TRUE_CMD", "FALSE_CMD", "gen_exec_result", "gen_exec_step_desc",
           "gen_rc_cmd", "gen_showenv_cmd", "gen_echo_cmd", "gen_output_cmd", "gen_sleep_cmd"]
