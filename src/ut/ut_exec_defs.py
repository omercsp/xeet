...
from xeet.core.result import TestResult, TestStatus, StepsListResult
import os
...
def gen_exec_result(status: TestStatus, **kwargs) -> TestResult:
    ...
    return TestResult(status=status, run_res=StepsListResult(results=[step_res_desc]))


GOOD_EXEC_RES = gen_exec_result(PASSED_TEST_STTS, rc=0, rc_ok=True, completed=True)
