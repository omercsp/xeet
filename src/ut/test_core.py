...
from ut.ut_exec_defs import gen_exec_step_desc, gen_sleep_cmd
...
def test_fetch_tests(xut: XeetUnittest):
    ...
    tests = xut.driver().fetch_tests(criteria)
    assert len(tests) == 6
    assert set([t.name for t in tests]) == {TEST0, TEST1, TEST2, TEST3, TEST4, TEST5}

    criteria.names = [TEST0, TEST1, TEST5]
    tests = xut.driver().fetch_tests(criteria)
    assert len(tests) == 3
    assert set([t.name for t in tests]) == set(criteria.names)

    criteria.names = [TEST0, TEST1, TEST5, "no such test"]
    tests = xut.driver().fetch_tests(criteria)
    assert len(tests) == 3
    assert set([t.name for t in tests]) == {TEST0, TEST1, TEST5}
...
def test_fetch_groups_list(xut: XeetUnittest):
    ...
    criteria.include_groups = list()
    criteria.names = [TEST0, TEST2, TEST3, "no such test"]
    tests = xut.driver().fetch_tests(criteria)
    assert len(tests) == 3
    assert set([t.name for t in tests]) == {TEST0, TEST2, TEST3}


def test_step_details(xut: XeetUnittest):
    test_desc = gen_dummy_step_desc(dummy_val0="var={test_var}", dummy_val1=10)
    ...
    step_details = step.details(full=False, printable=False)
    assert list(step_details.keys()) == ["step_type", "dummy_val1", "dummy_val0"]
    assert step_details["dummy_val0"] == "var={test_var}"
    assert step_details["dummy_val1"] == 10
    assert step_details["step_type"] == "dummy"

    step_details = step.details(full=False, printable=True)
    assert list(step_details.keys()) == ["Step type", "Dummy val1", "Printable dummy value 0"]
    assert step_details["Printable dummy value 0"] == "Printable var={test_var}"
    assert step_details["Dummy val1"] == "10"
    assert step_details["Step type"] == "dummy"

    step_details = step.details(full=True, printable=False)
    assert list(step_details.keys()) == ["name", "step_type", "base", "dummy_val1", "dummy_val0",
                                         "self_id"]
    assert step_details["dummy_val0"] == "var={test_var}"
    assert step_details["dummy_val1"] == 10
    assert step_details["step_type"] == "dummy"

    step_details = step.details(full=True, printable=True)
    assert list(step_details.keys()) == ["Name", "Step type", "Base", "Dummy val1",
                                         "Printable dummy value 0", "Step ID"]
    assert step_details["Printable dummy value 0"] == "Printable var={test_var}"
    assert step_details["Dummy val1"] == "10"
    assert step_details["Step type"] == "dummy"

    #  And again after setup
    test.setup(setup_steps=True)

    step_details = step.details(full=False, printable=False)
    assert list(step_details.keys()) == ["step_type", "dummy_val1", "dummy_val0"]
    assert step_details["dummy_val0"] == "var=var"
    assert step_details["dummy_val1"] == 10
    assert step_details["step_type"] == "dummy"

    step_details = step.details(full=False, printable=True)
    assert list(step_details.keys()) == ["Step type", "Dummy val1", "Printable dummy value 0"]
    assert step_details["Printable dummy value 0"] == "Printable var=var"
    assert step_details["Dummy val1"] == "10"
    assert step_details["Step type"] == "dummy"
...
def test_step_lists_inheritance(xut: XeetUnittest):
    ...
    def assert_test_model_steps(name: str,
        ...
        model = xut.driver().test_by_name[name].model
        assert isinstance(model, TestModel)
        assert_step_desc_list_equal(model.pre_run_steps(), pre_run)
        assert_step_desc_list_equal(model.run_steps(), run)
        assert_step_desc_list_equal(model.post_run_steps(), post_run)

    xut.add_test(TEST0, run=[DUMMY_OK_STEP_DESC])
    xut.add_test(TEST1, pre_run=[DUMMY_OK_STEP_DESC], base=TEST0)
    xut.add_test(TEST2, base=TEST1, run=[DUMMY_FAILING_STEP_DESC])
    xut.add_test(TEST3, base=TEST0, run_inheritance=StepsInheritType.Append.value,
                 run=[DUMMY_FAILING_STEP_DESC])
    xut.add_test(TEST4, base=TEST0, run_inheritance=StepsInheritType.Prepend.value,
                 run=[DUMMY_FAILING_STEP_DESC])
    xut.add_test(TEST5, base=TEST1, run=[], post_run=[DUMMY_OK_STEP_DESC, DUMMY_OK_STEP_DESC])
    xut.add_test(TEST6, base=TEST5, run=[DUMMY_FAILING_STEP_DESC],
                 run_inheritance=StepsInheritType.Append.value, save=True)

    assert_test_model_steps(TEST0, run=[DUMMY_OK_STEP_DESC])
    assert_test_model_steps(TEST1, pre_run=[DUMMY_OK_STEP_DESC], run=[DUMMY_OK_STEP_DESC])
    assert_test_model_steps(TEST2, pre_run=[DUMMY_OK_STEP_DESC], run=[DUMMY_FAILING_STEP_DESC])
    assert_test_model_steps(TEST3, run=[DUMMY_OK_STEP_DESC, DUMMY_FAILING_STEP_DESC])
    assert_test_model_steps(TEST4, run=[DUMMY_FAILING_STEP_DESC, DUMMY_OK_STEP_DESC])
    assert_test_model_steps(TEST5, pre_run=[DUMMY_OK_STEP_DESC],
...
def test_thread_support(xut: XeetUnittest):
    ...
    start = timer()
    results = xut.run_tests_list([TEST0, TEST1, TEST2], jobs=3)
