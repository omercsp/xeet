...
def test_fetch_test(xut: XeetUnittest):
    xut.add_test(TEST0)
    xut.add_test(TEST1, bad="bad", save=True)
    test = fetch_test(xut.file_path, TEST0)
    assert test is not None
    assert test.name == TEST0

    test = fetch_test(xut.file_path, TEST1)
    assert test is not None
    assert test.name == TEST1
    assert test.error != ""

    test = fetch_test(xut.file_path, TEST2)
    assert test is None
