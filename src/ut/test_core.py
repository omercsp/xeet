...
def test_step_model_inheritance(xut: XeetUnittest):
    ...
    model = xut.get_test(TEST0, init_phases=True).main_phase.steps[0].model
    assert isinstance(model, DummyStepModel)
    assert model.step_type == "dummy"
    assert model.dummy_val0 == "test"

    test = xut.get_test(TEST1, init_phases=True)
    assert test.error != ""

    model = xut.get_test(TEST2, init_phases=True).main_phase.steps[0].model
    assert isinstance(model, DummyStepModel)
    assert model.step_type == "dummy"
    assert model.dummy_val0 == "other_from_base"

    model = xut.get_test(TEST3, init_phases=True).main_phase.steps[0].model
    assert isinstance(model, DummyStepModel)
    assert model.step_type == "dummy"
    assert model.dummy_val0 == "other_from_base"
...
#  def xut.driver()fetch_tests(xut: XeetUnittest, criteria: TestsCriteria) -> list[Test]:
#      return xut.driver().fetch_tests(criteria=criteria)
#      return fetch_tests(xut.file_path, criteria)
...
def test_step_details(xut: XeetUnittest):
    ...
    test = xut.get_test(TEST0, init_phases=True)
    ...
    test = xut.get_test(TEST0, setup=True)
    assert test is not None
    step = test.main_phase.steps[0]
