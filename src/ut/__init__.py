...
from xeet.core.driver import xeet_driver, _XeetDriver
...
class ConfigTestWrapper:
    def __init__(self, name):
        ...
        if os.path.isabs(self.name):
            ...
            self.file_path = os.path.join(xeet_ut_dir_name(), self.name)
...
class XeetUnittest(ConfigTestWrapper):
    def __init__(self, name: str):
        super().__init__(name)
        self.reporters: list[EventReporter] = list()  # List of event reporters
    ...
    def run_tests(self, iterations: int = 1, **kwargs) -> RunResult:
        ...
        run_sttings = XeetRunSettings(criteria, iterations)
    ...
    def get_test(self, name, setup: bool = False, init_phases: bool = False) -> Test:
        tests = self.driver().fetch_tests(TestsCriteria(names=[name]), setup=setup,
    ...
    def driver(self) -> _XeetDriver:
        ret = xeet_driver(XeetSettings(file_path=self.file_path))
        for reporter in self.reporters:
            ret.rti.add_run_reporter(reporter)
        return ret
...
def gen_test_result(test: Test | None = None,
                    ...
                    **kwargs) -> TestResult:
    #  To creatre a test result, we need to update the phase results with the steps results.
    #  those must be initialized before the test result is created.
    if test is not None:
        assert test.phases_initialzed
