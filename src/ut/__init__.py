...
from xeet.core.xres import TestResult, XStepListResult
...
class XeetUnittest(unittest.TestCase):
    ...
    _res_comparisons: dict[type[XStepResult], _RES_COMPARISON_TYPE] = {}

    @classmethod
    def register_res_comparison(cls, res_type: type[XStepResult],
    ...
    def run_all_tests(cls, threads: int = 1) -> Iterable[TestResult]:
        ...
        run_info = run_tests(cls.main_config_wrapper.file_path, TestCriteria(), list(),
                             threads=threads)
        return [run_info.test_result(name, 0) for name in names]
    ...
    def assertTestResultEqual(self, res: TestResult, expected: TestResult) -> None:
        ...
        self.assertEqual(res.sub_status, expected.sub_status)
