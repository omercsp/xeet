...
#  result lsts are deepcopied to avoid the same object being used in mulitple phases, as
#  each step result is assigned to a step during the run.
def dup_step_res_list(res_list: list[StepResult]) -> list[StepResult]:
    return [deepcopy(res) for res in res_list]
