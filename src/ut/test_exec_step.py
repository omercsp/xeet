...
from xeet.core.xres import TestStatus, TestSubStatus
...
class TestExecStep(XeetUnittest):
    ...
    def test_simple_exec(self):
        ...
        expected = gen_exec_result(TestStatus.Passed, rc=0, rc_ok=True, completed=True)
        self.assertTestResultEqual(self.run_test(TEST0), expected)

        expected = gen_exec_result(TestStatus.Failed, rc=1, rc_ok=False, completed=True,
                                   failed=True)
        self.assertTestResultEqual(self.run_test(TEST1), expected)

        expected = gen_exec_result(TestStatus.NotRun, TestSubStatus.TestErr, rc=None,
    ...
    def test_allowed_rc(self):
        ...
        expected = gen_exec_result(TestStatus.Passed, rc=10, rc_ok=True, completed=True)
        self.assertTestResultEqual(self.run_test(TEST0), expected)
        self.assertTestResultEqual(self.run_test(TEST1), expected)

        expected = gen_exec_result(TestStatus.Failed, rc=10, rc_ok=False, completed=True,
                                   failed=True)
        self.assertTestResultEqual(self.run_test(TEST2), expected)

        expected = gen_exec_result(TestStatus.Passed, rc_ok=True, completed=True)
        for _ in range(5):
    ...
    def test_output_behavior(self):
        ...
        expected = gen_exec_result(TestStatus.Failed, rc=0, rc_ok=True, completed=True, failed=True,
