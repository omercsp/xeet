...
_step_args = XStepTestArgs(stage_prefix="ut")
...
class TestExecStep(XeetUnittest):
    ...
    def assertExecStep(self, step: ExecStep, expected: ExecStepResult,
                       xvars: XeetVars = default_xvars, **kwargs) -> None:
        step.expand(xvars)
    ...
    def test_simple_exec(self):
        ...
        result = _gen_exec_step_result(step, rc=0, rc_ok=True, completed=True)
    ...
    def test_allowed_rc(self):
        ...
        step = _gen_exec_step(desc)
        result = _gen_exec_step_result(step, rc=10, rc_ok=True, completed=True)
        self.assertExecStep(step, result)
    ...
    def test_cwd(self):
        ...
        desc = _gen_exec_step_desc(cmd=_PWD_CMD, cwd=cwd, expected_stdout=f"{cwd}\n")
        step = _gen_exec_step(desc)
        self.assertExecStep(step, _gen_exec_step_result(step, completed=True, rc=0, rc_ok=True))
    ...
    def test_env(self):
        desc = _gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} TEST_ENV", env={"TEST_ENV": "test"},
                                   expected_stdout="test\n")
        step = _gen_exec_step(desc)
        self.assertExecStep(step, _gen_exec_step_result(step, completed=True, rc=0, rc_ok=True))

        desc = _gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} NOSUCHVAR", expected_stdout="\n")
        step = _gen_exec_step(desc)
        expected = _gen_exec_step_result(step, completed=True, rc=0, rc_ok=True)
        self.assertExecStep(step, expected)

        os.environ["OS_TEST_ENV"] = "os test"
        desc = _gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} OS_TEST_ENV", use_os_env=True,
        ...
        with tempfile.NamedTemporaryFile() as tmpfile:
            ...
            desc = _gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} var0", env_file=tmpfile.name,
    ...
    def test_shell_usage(self):
        ...
        desc = _gen_exec_step_desc(cmd=cmd, use_shell=True, expected_stdout=shell_expected_stdout)
        step = _gen_exec_step(desc)
        expected = _gen_exec_step_result(step, completed=True, rc=0, rc_ok=True)
        self.assertExecStep(step, expected)
    ...
    def test_output_behavior(self):
        ...
        desc = _gen_exec_step_desc(cmd=cmd, expected_stdout="OEOE")
        step = _gen_exec_step(desc)
        expected = _gen_exec_step_result(step, completed=True, rc=0, rc_ok=True)
        self.assertExecStep(step, expected)
