...
class TestExecStep(XeetUnittest):
    ...
    def test_env(self):
        ...
        step_desc = gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} var0", env_file=env_file_path,
                                       expected_stdout="val0\n")
        self.add_test(_TEST0, run=[step_desc], reset=True)

        step_desc = gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} var1", env_file=env_file_path,
                                       expected_stdout="val1\n")
        self.add_test(_TEST1, run=[step_desc])

        step_desc = gen_exec_step_desc(cmd=f"{_SHOWENV_CMD} var2", env_file=env_file_path,
                                       expected_stdout="\n")
    ...
    def test_output_filter(self):
        ...
        cmd = f"{_ECHOCMD} --no-newline abcdef"
