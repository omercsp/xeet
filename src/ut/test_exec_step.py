...
from xeet.steps.exec_step import ExecStepModel, _OutputBehavior
...
from xeet.core.result import TestResult, StepsListResult
import tempfile
import os
import json
...
class TestExecStep(XeetUnittest):
    ...
    def test_simple_exec(self):
        ...
        expected = gen_exec_result(FAILED_TEST_STTS, rc=1, rc_ok=False, completed=True,
                                   failed=True)
        self.assertTestResultEqual(self.run_test(TEST1), expected)

        expected = gen_exec_result(
            TestStatus(TestPrimaryStatus.NotRun, TestSecondaryStatus.TestErr), rc=None,
            os_error=OSError(), completed=False)
    ...
    def test_var_expansions(self):
        ...
        expected = TestResult(status=init_err_status, run_res=StepsListResult())
    ...
    def test_allowed_rc(self):
        ...
        expected = gen_exec_result(PASSED_TEST_STTS, rc=10, rc_ok=True, completed=True)
        self.assertTestResultEqual(self.run_test(TEST0), expected)
        self.assertTestResultEqual(self.run_test(TEST1), expected)

        expected = gen_exec_result(FAILED_TEST_STTS, rc=10, rc_ok=False, completed=True,
                                   failed=True)
        self.assertTestResultEqual(self.run_test(TEST2), expected)

        expected = gen_exec_result(PASSED_TEST_STTS, rc_ok=True, completed=True)
        for _ in range(5):
            self.assertTestResultEqual(self.run_test(TEST3), expected)
    ...
    def test_timeout(self):
        step_desc = gen_exec_step_desc(cmd=gen_sleep_cmd(1), timeout=0.5)  # 1 second sleep
        self.add_test(TEST0, run=[step_desc], reset=True, save=True)
        expected = gen_exec_result(
            TestStatus(TestPrimaryStatus.NotRun, TestSecondaryStatus.TestErr), rc=None,
            completed=False, timeout_period=True)
        self.assertTestResultEqual(self.run_test(TEST0), expected)
    ...
    def test_env(self):
        ...
        self.assertTestResultEqual(self.run_test(TEST0), GOOD_EXEC_RES)
        self.assertTestResultEqual(self.run_test(TEST1), GOOD_EXEC_RES)
        self.assertTestResultEqual(self.run_test(TEST2), GOOD_EXEC_RES)
        self.assertTestResultEqual(self.run_test(TEST3), GOOD_EXEC_RES)
    ...
    def test_output_behavior(self):
        ...
        expected = gen_exec_result(FAILED_TEST_STTS, rc=0, rc_ok=True, completed=True, failed=True,
                                   output_behavior=_OutputBehavior.Split, stdout_diff="yes")
        self.assertTestResultEqual(self.run_test(TEST5), expected)
        expected = gen_exec_result(FAILED_TEST_STTS, rc=0, rc_ok=True, completed=True, failed=True,
                                   output_behavior=_OutputBehavior.Split, stderr_diff="yes")
        self.assertTestResultEqual(self.run_test(TEST6), expected)
